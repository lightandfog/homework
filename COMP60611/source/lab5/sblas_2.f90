...
! be around 1/pth of the total memory requirement)
  ...
  real*8 a(ns),b(ns),x(ns),y(ms),l(ms,ms)
...
! Now calculate the sum of the partial results 
     ...
     glob_res = 0
     call MPI_REDUCE(res, glob_res, 1, MPI_DOUBLE_PRECISION, &
...
! Get a copy of a for each processor.
     ...
     call MPI_ALLGather(MPI_IN_PLACE, nnx, MPI_DOUBLE_PRECISION, a, nnx, &
...
! print out some results
  ...
  do i=1,numprocs
    ...
    if ( myid==(i-1) ) then
      ...
      print *,'x(nx) = ', x(nx), 'myid = ', myid
      print *,'y(mx) = ', y(mx), 'myid = ', myid
